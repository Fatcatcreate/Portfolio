...
![equation](https://latex.codecogs.com/svg.latex?\bg_white\LARGE%20Q=\frac{1}{2m}\sum_{i=1}^{N}\sum_{j=1}^{N}\Big(A_{ij}-\frac{k_i\,k_j}{2m}\Big)\,\delta(c_i,c_j))

---

Adjacency matrix

![equation](https://latex.codecogs.com/svg.latex?\bg_white\LARGE%20A_{ij})
...
![equation](https://latex.codecogs.com/svg.latex?\bg_white\LARGE%20k_i=\sum_{j=1}^{N}A_{ij})

This is the number of edges connected to node i.

Total number of edges

![equation](https://latex.codecogs.com/svg.latex?\bg_white\LARGE%20m=\frac{1}{2}\sum_{i=1}^{N}k_i)

This counts each edge once (since the sum of degrees counts each edge twice).

Expected edges in random network

![equation](https://latex.codecogs.com/svg.latex?\bg_white\LARGE%20\frac{k_i\,k_j}{2m})

This is the expected number of edges between nodes i and j in a random network with the same degree distribution.

Community indicator

![equation](https://latex.codecogs.com/svg.latex?\bg_white\LARGE%20\delta(c_i,c_j))
...
This modularity measure is widely used in **community detection** algorithms to find the most meaningful partitions in networks and compare different community structures.
